...
function createAndShowForm(player) {
    const title = new ObservableString(" ", { clientWritable: true });
    const label = new ObservableString(`§f█§f█§f█§m█§m█§m█§m█§m█§f█§f█§f█§f█
...
Command.registerCommand({ name: `${PACK_PREFIX}custom_form`, description: "", permissionLevel: CommandPermissionLevel.Admin }, async (origin) => {
