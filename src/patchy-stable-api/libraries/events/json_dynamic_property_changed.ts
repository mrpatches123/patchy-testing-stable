...
export class JSONDynamicPropertyChangedEvent extends StorageChangedEventTemplate {
	...
	subscribe(callback: (data: { target: Player | Entity | World, key: string, previousValue: any | undefined; currentValue: any | undefined, cancel: boolean, cancelCache: Boolean, cancelSet: Boolean; }) => void) {
		const subscribeId = this.currentSubscribeId++;
		this.subscriptions[subscribeId] = callback;
		this.currentSubscribes++;
		return subscribeId;
	}
	runEvent(target: Player | Entity | World, key: string, previousValue: any | undefined, currentValue: any | undefined): { target: Player | Entity | World, key: string, previousValue: any | undefined, currentValue: any | undefined, cancel: boolean, cancelCache: Boolean, cancelSet: Boolean; } {
