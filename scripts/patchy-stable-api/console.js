...
export function jsonifyMap(map) {
    const object = {};
...
function consoleStringify(arg) {
    if (arg instanceof Error)
        return arg;
    if (arg instanceof Map)
        return JSON.stringify(jsonifyMap(arg));
...
const argsReformat = (consoleFunction, args) => {
    ...
    if (logType === "none")
        return;
    if (logType === "specific" && !args?.[0]?.includes?.(`~${logSpecificKey}~`))
        return;
    if (!args || !args.length)
        consoleFunction();
    if (!shouldReformat)
        return consoleFunction(args);
...
export async function showConsoleForm(receiver) {
    ...
    return new ModalForm()
        ...
        if (logType === "specific")
            worldStorage.setString("logSpecificKey", `${data}`);
